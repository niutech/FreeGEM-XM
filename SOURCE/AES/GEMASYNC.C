...
EXTERN UWORD	button, xrat, yrat, kstate;
	...
	REG LONG	aparm;
{
	...
	switch(afunc)
	{
	  case MU_KEYBD: akbin(e,aparm);	break;
	...
	REG EVSPEC	mask;
{
	...
	for (p = (q = (EVB *) &rlr->p_evlist) -> e_nextp; p;
	  ...
	  if (p->e_mask == mask) 
	    break;
						/* get the returns from	*/
						/*   the event		*/
/*
	if (prets != NULLPTR)
	{
	  prets[0] = LHIWD(p->e_parm);
	  prets[1] = LLOWD(p->e_parm);
	  prets[2] = LHIWD(p->e_return);
	  prets[3] = kstate;
	}
*/
						/* found the event,	*/
