	...
	WORD		*pcount;
{
	...
	WORD		err_code, fs_index;
	REG WORD	i, j, gap, drv;
	BYTE		*temp;
...
*/
	WORD
fs_newdir(ftitle, fpath, pspec, tree, pcount)
	LONG		ftitle;
	LONG		fpath;
	BYTE		*pspec;
	LONG		tree;
	WORD		*pcount;
{
	...
	LBSET(ftitle, ' ');
	ftitle++;
	ftitle += LSTCPY(ftitle, ADDR(pspec));
	LBSET(ftitle, ' ');
	ftitle++;
	LBSET(ftitle, NULL);
	ptmp = &gl_fsobj[0];
	...
	WORD		*pbutton;
{
	...
	WORD		curr, count, sel;
	WORD		mx, my, xoff, yoff;
	REG LONG	tree;
	LONG		spec, ad_fpath, ad_fname, ad_ftitle, ad_locstr;
	WORD		fname_len, fpath_len, ftitle_len; 
	WORD		dclkret, cont, firsttime, newname;
	REG BYTE	*pstr, *pspec;
	GRECT		pt;
	BYTE		locstr[64];
	...
	fs_sset(tree, FTITLE, ADDR(" *.* "), &ad_ftitle, &ftitle_len);
	fs_sset(tree, FSDIRECT, pipath, &ad_fpath, &fpath_len);
	...
	while( cont )
	{
	  ...
	  if ( !strcmp(&D.g_dir[0], &locstr[0]) )
	  {
	    ...
	    strcpy("*.*", pstr);
	    fs_newdir(ad_ftitle, ad_locstr, pspec, tree, &count);
	    curr = sel = touchob = 0;
	    firsttime = FALSE;
	  }
