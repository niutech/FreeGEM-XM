...
LONG	obspec ;
    ...
    tool_mfdb.mp = LLGET( obspec ) ;
    tool_mfdb.fh = LWGET( obspec + 4 ) ;
    tool_mfdb.fww = LWGET( obspec + 6 ) >> 1 ;
    tool_mfdb.fwp = tool_mfdb.fww << 4 ;
    tool_mfdb.ff = 1 ;
    vrn_trnfm(&tool_mfdb, &tool_mfdb);
} /* init_icon */
...
/* initialize libraries	*/
	...
	if (gl_height <= 300)
	{
	  ...
	  init_icon(templn, DEICON);
	}
	init_icon(templn, DENAME1);
	init_icon(templn, DENAME2);
	init_icon(templn, DENAME3);
	init_icon(templn, DENAME4);
#endif
